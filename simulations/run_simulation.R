...
run_simulation <- function(params, beta_bin, beta_con, n_sims, alpha = 0.05, verbose = F, future_seeds) {
  ...
  ### MultiPhen
  cat(ifelse(verbose, 'Running Multiphen\n', ''))
  if(!params$healthy_controls) {
    p_multiphen <- future_map_dbl(datasets, ~multi_phen(.x))
    p_values$p_multiphen <- p_multiphen
  } else {
    p_values$p_multiphen <- NA
  }
  ...
  df_power <-
    p_values %>%
    summarise(across(.cols = -c('p_pheWAS_mega', 'p_pheWAS_meta', 'p_multiphen'), .fns = ~mean(.x <= alpha))) %>%
    ...
    bind_cols(summarise(p_values, across(.cols = c('p_multiphen'), .fns = ~mean(.x <= alpha/params$num_sites)))) %>%
    bind_cols(df_info) %>%
    bind_cols(df_ep)
